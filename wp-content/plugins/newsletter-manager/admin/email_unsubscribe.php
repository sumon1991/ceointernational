...
if (
		...
		|| ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'email_unsubscribe_'.$xyz_em_emailId )
) {
	wp_nonce_ays( 'email_unsubscribe_'.$xyz_em_emailId );
...
if(count($emailCount) == 0){
	...
	if($xyz_em_search=='')
	{
		...
		?>
		<script>
		document.location.href="admin.php?page=newsletter-manager-manage-emails&emailmsg=2&pagenum=<?php echo $xyz_em_pageno ?>";
		</script>
		<?php
